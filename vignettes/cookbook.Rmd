...
We'll be using a dataset which gives the population with an income below the 
poverty level in the last 12 months for a selection of RIN members
```{r}
data(cori_poverty)
...
cori_poverty %>%
  ggplot2::ggplot(aes(estimate_below_poverty_level_2019, reorder(rin_community, estimate_below_poverty_level_2019))) +
  ggplot2::geom_col(fill = cori_colors["Emerald"]) +
  geom_text(
    aes(label = scales::number(estimate_below_poverty_level_2019, accuracy = 1, big.mark = ",")),
    hjust = -.1,
    family = "Montserrat"
  ) +
...
```{r, fig.asp = 0.8, fig.width = 5, warning = FALSE, message = FALSE}

...
# We'll make a map highlighting cori communities
...
counties <- tigris::counties(cb = TRUE, year = 2019)
...
dta <- dplyr::left_join(
  ...
  sf::st_as_sf() %>%
  # If you have data for Alaska or Hawaii you'll want to
  # shift Alaska and Hawaii underneath the lower 48
...
# Most of the time the default limits for the data will work,
# but if you want scope the limits
# of the map window you can take the following steps

# 1) Specify the lower left and upper right corners in WGS84 
us_limits_wgs84 <- sf::st_sfc(
  sf::st_point(c(-120, 0)),
  sf::st_point(c(-65, 35)),
  crs = 4326
)
...
# 3) Pull new coordinates in Albers
us_limits_coord <- sf::st_coordinates(us_limits_albers)

dta %>%
  ggplot(aes(fill = state_abbr)) + 
  # State background
  geom_sf(data = states, fill = "#d0d2ce", color = "white", size = .0) +
  ...
  ggplot2::theme(
    # Hide legend of different states
    legend.position = "none"
  ) +
  ggplot2::labs(
    title = "Counties with CORI communities",
    subtitle = "2019",
    y = NULL,
    x = NULL,
    caption = NULL
